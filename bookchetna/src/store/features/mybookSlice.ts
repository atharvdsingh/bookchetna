...
const Mybookslice=createSlice({
    ...
    reducers:{
        ...
        visibilityStatusChanged:(state,action:PayloadAction<SerializableBook>)=>{
            state.myallBooks=state.myallBooks.filter((book)=>book.visibilityStatus!=action.payload.visibilityStatus)
        }
    }
})
